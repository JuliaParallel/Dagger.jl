...
if [[ "$MODE" == "mpi" ]]; then
  ...
  exec mpiexec --hostfile "$RANKFILE" -n "$N" \
    ...
    -x LINALG_BENCH_SWEEP -x LINALG_BENCH_SWEEP_KEYS \
    -x LINALG_BENCH_TILES -x LINALG_BENCH_ASSIGNS \
    -x LINALG_BENCH_LAYOUTS -x LINALG_BENCH_SCOPES \
    -x LINALG_BENCH_TILE -x LINALG_BENCH_ASSIGN \
    -x LINALG_BENCH_LAYOUT -x LINALG_BENCH_SCOPE \
