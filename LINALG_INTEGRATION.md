...
| Native GPU AMG setup / apply | **Partial** | AlgebraicMultigrid.jl is still host: setup host-stages *tiles* inside a GPU `ExactScope`. V-cycle Jacobi / ℓ1-Jacobi / Chebyshev / SpMV keep Krylov vectors in VRAM (`_pc_alloc_vec` / `AllocateUndef`, not a host `Vector` restamp). Hybrid GS, per-tile `AMGPreconditioner`, RAS, and the coarsest LU Adapt a temporary inside the GPU-scoped task (lesson 35 / 52). Not HYPRE device PMIS + ext+i. |
