...
| P0 | hypre-alike | done | `linalg/hypre-alike` @ `ed51d132` | Remaining HYPRE-alike GlobalAMG as opt-in kwargs (defaults stay HMIS / SA / V / Jacobi). Falgout / CLJP / CGC / aggressive coarsen; extended / ext+i / FF / AIR / multipass interp; additive / multadditive (`1/n` damp); FSAI; `pmax` / `trunc_factor` / `coarse_drop`; ComplexF64 + GMRES. Lesson 53. Per-tile `AMGPreconditioner` unchanged. AWS job `e500a33db70fa105`: GlobalAMG 261, NNS 44, GMG 158, iterativesolvers 460; full `array/linalg` (no Finch) green. First run hit `_pc_undef_vec` (GPU helper not on the feature branch); rerun after `ed51d132`. | 2026-09-10 |
...
| 2026-09-10 | `a68c1503` `linalg/gpu-amg` @ `dda18291` | No conflicts. Lesson 52. ROC smoke 30/30; CUDA smoke 30/30 on `i-029babd72a0984f4b` (`g4dn.xlarge`, label `linalg-gpu-amg-cuda`, terminated). Kept `_solve_pinned_dvector`. Pitch-videos `72714ecd` did not edit this file. HYPRE-alike unioned in `cd1c8797`. |
| 2026-09-10 | `cd1c8797` `linalg/hypre-alike` @ `ed51d132` | Conflicts: `AGENTS.md` (kept GPU lesson 52 extras + incoming 53), `LINALG_INTEGRATION.md` / `docs/src/iterative-solving.md` (union GPU apply details + incoming HYPRE table), `ext/AlgebraicMultigridExt.jl` (kept `_pc_alloc_vec` / `_pc_undef_vec` / `_amg_host_scope` / Int32 CSC + incoming coarsen/interp/FSAI). Kept `_solve_pinned_dvector`. AWS `e500a33db70fa105`: GlobalAMG 261, NNS 44, GMG 158, iterativesolvers 460; full `array/linalg` (no Finch) green. |
...
`AGENTS.md` lessons 27–53 are the union (through dense `schur` 47; BLAS-1 is 48; `@einsum` is 49; BSR is 50; BoomerAMG-alike PMIS is 51; GPU AMG VRAM is 52; remaining HYPRE-alike is 53). Lesson 20 remains unused (pre-existing gap). Lesson 35 is GPU-PC; do not reuse that number.
...
| Coarsening: HMIS / PMIS / Falgout / CLJP / CGC / aggressive | **Yes (opt-in extras)** | Default stays `coarsen=:hmis`. Full `:pmis` / `:falgout` / `:cljp` / `:aggressive` are opt-in — they lose the 1-D n=128 Jacobi gate (Falgout ~2.24, CLJP ~2.15, aggressive ~1.73, PMIS ~1.81 vs HMIS ~0.93 / Jacobi ~0.99). `:cgc` beats Jacobi on 1-D and 2-D. All four new coarseners beat Jacobi on 2-D 8×8. `:standard` is the old leftover-pair path. Do not merge already-assigned interface aggregates. |
| Interpolation: classical / extended / ext+i / FF / AIR / multipass | **Yes (opt-in)** | Default SA `:sa` / RS `:direct`. `interp=:extended` / `:exti` (`:extended_i` / `Symbol("ext+i")`) / `:ff` / `:multipass` / `:air` stay distributed (C-neighbor map or one-point AIR `R`; no collect of fine `A`). Classical interps lose 1-D n=64 vs Jacobi and win on 2-D 8×8. |
| Smoothers: hybrid GS, Schwarz, Chebyshev, ILU, FSAI, ℓ1-Jacobi | **Yes** | `smoother=:jacobi` (default, `relax=2/3`), `:l1jacobi`, `:chebyshev`, `:hybrid_gs`, `:ilu` / `:ras`, `:fsai` (block-diagonal `G'G` of each diagonal tile; not a new solver type). FSAI loses 1-D n=64 (~1.95 vs Jacobi ~0.98) and beats Jacobi on 2-D 8×8 (~0.11 vs ~0.78). |
| Cycle types: W, F, additive / mult-additive AMG | **Yes** | `cycle=:v` (default), `:w`, `:f`, `:additive` (fine smoother + `1/n`-damped coarsest of `R b`), `:multadditive` (pre-smooth, damped coarsest of the updated residual, post-smooth). Unscaled `R b` overshoots (~7.7 vs Jacobi ~0.98 on 1-D n=64). A residual line search makes `mul!` nonlinear. Prefer `:v` for Krylov. |
| Strength threshold / truncation / `Pmax` / non-Galerkin drop | **Yes** | AlgebraicMultigrid.jl `strength=` / `aggregate=` passthrough, plus first-class `pmax` / `trunc_factor` (`truncation` alias) / `coarse_drop`. Zero means off (no silent HYPRE default). These change `nnz(P)` / `nnz(Ac)`. |
| Complex arithmetic | **Yes** | `ComplexF64` `DMatrix` + `GlobalAMG` + `Krylov.gmres`. Strength uses `abs` of entries; vector `fit_candidates` uses a real Euclidean norm (AMG.jl's `>` on Complex norms throws). Real path stays the default. |
...
**Covered (short):** global V/W/F/additive/mult-additive cycle with tiled HMIS-lite `P` and distributed RAP; opt-in PMIS / Falgout / CLJP / CGC / aggressive; extended / ext+i / FF / AIR / multipass interp (distributed); Jacobi / ℓ1-Jacobi / Chebyshev / hybrid GS / ILU / RAS / FSAI level smoothers; `pmax` / `trunc_factor` / `coarse_drop`; ComplexF64 `GlobalAMG` + GMRES; SA near-nullspace (`N` only) and `blocksize`; geometric RAP V-cycle; RAS as `PCASM`; per-tile AMG as Schwarz (do not call that BoomerAMG).

**Missing (short):** fully device-side AMG setup (tiles still host-stage); HYPRE device PMIS + ext+i; a default change away from HMIS (nothing beat HMIS on *both* 1-D n=128 and 2-D).
