...
Last coordinator pass: 2026-09-07 (`linalg/mixed-precision` merged; remaining P0 leftover and other P1 workstreams still in progress on siblings).
...
P0 leftovers still **in progress** on siblings (do not merge those branches here):
...
`AGENTS.md` lessons 27–41 are the union of the per-workstream lesson 27s (LinearSolve `DefaultLinearSolver`; ASM `:restrict` / `:basic`; GlobalAMG vs per-tile residual; qualify `cholesky!`/`mul!`; `SparseCOOBucket` not in datadeps; Projected orthonormalize / `Adjoint` `mul!`; `hvcat` is not `MatNest`; GPU block-PC `ProcessScope` gather; sparse `lu!(F, A)` / PureUMFPACK cannot cheap-refactor; `@stencil` cannot express GMG transfers; `view(::DArray)` stays `SubArray`; near-nullspace on the GlobalAMG constructor; graph partition is `partitioner=`/`perm=` on geometric `Blocks`, not `Dagger.metis`; mixed-eltype `mul!` / FP32 PC apply). Lesson 20 remains unused (pre-existing gap).
