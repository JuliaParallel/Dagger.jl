...
"""
    clear_gpu_stream_syncdeps!()

Clear per-backend GPU stream sync state (producer→stream maps, event pools)
after a datadeps region completes. No-op for backends that do not implement it.
"""
function clear_gpu_stream_syncdeps!()
    for kind in (:CUDA, :ROC, :oneAPI, :Metal, :OpenCL)
        if gpu_can_compute(kind)
            clear_gpu_stream_syncdeps!(Val(kind))
        end
    end
end
clear_gpu_stream_syncdeps!(::Val) = nothing
