...
function _pretty_time(t; digits=3)
    r(t) = round(t; digits)
    if t > 1000^3
        "$(r(t/(1000^3))) s"
    elseif t > 1000^2
...
function logs_to_dot(logs::Dict; disconnected=false, color_by=:fn,
    ...
    for v in con_vs
        ...
        if times
            tid = vertex_to_tid[v]
            start_time = _pretty_time(start_times[tid]; digits=times_digits)
            finish_time = _pretty_time(finish_times[tid]; digits=times_digits)
