...
### Raw-bytes transport decomposition

# Hooks for MPIExt's out-of-place raw-bytes path: decompose a value into
# contiguous bitstype buffers plus a small serializable header, so the payload
# travels as MPI buffers instead of a full Julia `serialize`. Same style as
# `move_rewrap_parts`/`move_rewrap_build`; a type that opts out (the default)
# falls back to serialization, so these are purely a transport optimization.
#
# Declared in core, rather than in MPIExt, so that container-specific methods
# live in the extension that owns the container. `SparseMatrixCSC` support, for
# instance, belongs to `MPISparseExt` (triggered on MPI *and* SparseArrays) --
# putting it in MPIExt would make MPI acceleration require SparseArrays to be
# loaded at all. This matches the MPI/GPU interop hooks in `gpu.jl`.

# Decompose `x` -> `(parts::Tuple, header)`, where each part is a `DenseArray`
# with a bitstype eltype. Return `nothing` to opt out of the raw-bytes path.
inplace_mpi_parts(x) = nothing
# Allocate the receive buffers for `T` from `header` (host-resident; the sender
# host-stages device arrays, and callers convert to the destination space).
inplace_mpi_alloc(::Type{T}, header) where T =
    error("inplace_mpi_alloc not defined for $T")
# Rebuild a value of type `T` from received `parts` and `header`.
...
# allocations present in our objectid registry, so this is a no-op for plain
# `Array`s.
