...
import ..Dagger
import ..Dagger: Context, Processor, SchedulerOptions, Options, Thunk, WeakThunk, ThunkFuture, ThunkFailedException, Chunk, WeakChunk, OSProc, AnyScope, DefaultScope, LockedObject, Argument, Signature
import ..Dagger: order, dependents, noffspring, istask, inputs, unwrap_weak_checked, wrap_weak, affinity, tochunk, timespan_start, timespan_finish, procs, move, chunktype, default_enabled, processor, get_processors, get_parent, execute!, rmprocs!, thunk_processor, constrain, cputhreadtime, maybe_take_or_alloc!
...
function schedule!(ctx, state, sch_options, procs=procs_to_use(ctx, sch_options))
    lock(state.lock) do
        ...
        # Schedule tasks
        to_fire = @reusable_dict :schedule!_to_fire ScheduleTaskLocation Vector{ScheduleTaskSpec} ScheduleTaskLocation(OSProc(), OSProc()) ScheduleTaskSpec[] 32
...
function fire_tasks!(ctx, task_loc::ScheduleTaskLocation, task_specs::Vector{ScheduleTaskSpec}, state)
    ...
    for task_spec in task_specs
        ...
        # Clear un-serializable Options fields
...
function (ets::FireTaskSpec)()
    ...
    @maybelog ctx timespan_start(ctx, :fire, (;worker=pid), nothing)
    try
        ...
        @maybelog ctx timespan_finish(ctx, :fire, (;worker=pid), nothing)
    end
    return
end
...
function start_processor_runner!(istate::ProcessorInternalState, uid::UInt64, return_queue::RemoteChannel)
    ...
    proc_run_task = @task begin
        ...
        while isopen(return_queue)
            ...
            end "thunk $thunk_id" DoTaskSpec(to_proc, return_queue, task)
            lock(istate.queue) do _
                tasks[thunk_id] = t
