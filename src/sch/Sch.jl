...
function start_state(deps::Dict, node_order, chan)
    ...
    for k in sort(collect(keys(deps)), by=node_order)
        if istask(k)
            waiting = Set{Thunk}(Iterators.map(syncdep->unwrap_weak_checked(something(syncdep.thunk)), k.options.syncdeps))
            if isempty(waiting)
                push!(state.ready, k)
            else
...
function proc_state(uid::UInt64, proc::Processor)
    states = proc_states(uid)
    state = MemPool.lock_read(states.lock) do
        return get(states.dict, proc, nothing)
    end
    if state === nothing
        state = MemPool.lock(states.lock) do
            state = ProcessorState(ProcessorInternalState(), Task())
            states.dict[proc] = state
            return state
        end
    end
