...
unwrap_nested_exception(err::Dagger.ThunkFailedException) =
    unwrap_nested_exception(err.ex)
...
function store_result!(state, thunk, value; error::Bool=false)
    ...
    thunk.finished = true
    if error && value isa Exception && !(value isa ThunkFailedException)
        thunk.cache_ref = Some{Any}(ThunkFailedException(thunk, thunk, value))
    else
