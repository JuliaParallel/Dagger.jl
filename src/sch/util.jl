...
function has_capacity(state, p, gp, time_util, alloc_util, occupancy, sig; snap=nothing)
    T = typeof(p)
...
@reuse_scope function estimate_task_costs!(sorted_procs, costs, state, procs, task; sig=nothing, snap=nothing)
