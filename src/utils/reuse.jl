...
function Base.push!(list::ReusableLinkedList{T}, value::T) where T
...
function Base.map!(f, list_out::ReusableLinkedList{T}, list_in::ReusableLinkedList{V}) where {T,V}
    @assert length(list_out) == length(list_in) "lists must have the same length"
...
mutable struct ReusableTaskCache
    ...
    function ReusableTaskCache(N::Integer)
        ...
        chans = Vector{Channel{Function}}(undef, N)
        ready = [Threads.Atomic{Bool}(true) for _ in 1:N]
        for idx in 1:N
            chans[idx] = Channel{Function}(0)
...
function reusable_task_loop(chan::Channel{Function}, ready::Threads.Atomic{Bool})
    r = rand(1:128)
    while true
        f = try
            take!(chan)
...
function (cache::ReusableTaskCache)(f::Function, name::String)
