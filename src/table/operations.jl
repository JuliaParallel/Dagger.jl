...
function reduce(f, d::DTable; cols=nothing::Union{Nothing, Vector{Symbol}}, init=Base._InitialValue())
    if length(d.chunks) > 0
        # this columnnames thing is overly complicated, because it's supposed to be a vector and normally it's a tuple
        # TODO: make this nice in tablesjl PR
        columns = cols === nothing ? [c for c in Tables.columnnames(Tables.columns(_retrieve(d.chunks[1])))] : cols
    else
...
function reduce(
    ...
    if length(gd.dtable.chunks) > 0
        # this columnnames thing is overly complicated, because it's supposed to be a vector and normally it's a tuple
        # TODO: make this nice in tablesjl PR
        columns = cols === nothing ? [c for c in Tables.columnnames(Tables.columns(_retrieve(gd.dtable.chunks[1])))] : cols
    else
        return Dagger.@spawn NamedTuple()
    end
