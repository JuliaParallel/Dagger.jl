...
function unsafe_free!(A::DArray)
    # Free each chunk directly. We deliberately do NOT route this through
    # `spawn_datadeps`/`@spawn`: spawning a task per chunk passes the chunk back
    # through the scheduler (buffered copy + retained task/argument state), which
    # frees a copy while pinning the original — so the DArray's memory was never
    # actually reclaimed. Calling `unsafe_free!(chunk)` directly returns the
    # backing memory immediately.
    for chunk in A.chunks
        c = chunk isa DTask ? fetch(chunk; raw=true) : chunk
        unsafe_free!(c)
    end
    return
end
