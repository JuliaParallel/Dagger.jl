...
ctranspose(x::AbstractChunk) = delayed(ctranspose)(x)

transpose(x::LazyArray) = Transpose(transpose, x)
transpose(x::AbstractChunk) = delayed(transpose)(x)
...
(*)(a::AbstractChunk, b::AbstractChunk) = delayed(*)(a,b)
(+)(a::AbstractChunk, b::AbstractChunk) = delayed(+)(a,b)
...
function _scale(l, r)
    res = similar(r, Any)
    for i=1:length(l)
        res[i,:] = map(x->delayed((a,b) -> Diagonal(a)*b)(l[i], x), r[i,:])
    end
    res
end
