...
function select_neighborhood_info(chunks, idx, neigh_dist, boundary)
    ...
    N = ndims(chunks)
    chunk_dist = 1
    # Concretely typed: every element has this exact shape, and an abstract
...
# chunks repeatedly: every chunk is some neighbor's neighbor 3^N-1 times over.
# Memoize by identity so each distinct chunk is resolved once per sweep.
function _memoized_memory_space!(memo::IdDict{Any,Any}, @nospecialize(chunk))
    space = get(memo, chunk, nothing)
