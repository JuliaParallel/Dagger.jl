...
function searchmax_pivot!(piv_idx::AbstractArray{Int}, piv_val::AbstractArray{Float64}, A::AbstractArray{T}, offset::Int=0) where T
    max_idx = LinearAlgebra.BLAS.iamax(A[:])
    piv_idx[1] = offset+max_idx
    piv_val[1] = A[max_idx]
end

function update_ipiv!(ipivl, piv_idx::AbstractArray{Int}, piv_val::AbstractArray{Float64}, k::Int, nb::Int)
...
function update_panel!(M::AbstractArray{T}, A::AbstractArray{T}, p::Int) where T
    ...
    LinearAlgebra.BLAS.ger!(-1.0, view(M, :, p), view(A, p, p+1:size(A,2)), view(M, :, p+1:size(M,2)))
end
