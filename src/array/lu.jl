...
const _StridedVecOrMat{T} = Union{StridedVector{T},StridedMatrix{T}}
...
function swaprows_trail!(A::AbstractMatrix{T}, M::AbstractMatrix{T}, ipiv::AbstractVector{Int}, m::Int, mb::Int) where T
    if A isa _StridedVecOrMat && M isa _StridedVecOrMat
...
function _lu_rowmax_tiled!(A::DMatrix{T}; check::Bool = true, allowsingular::Bool = false) where {T<:LinearAlgebra.BlasFloat}
    ...
    maybe_copy_buffered(A => Blocks(min_mb_nb, min_mb_nb)) do A
        ...
        # original fine-grained, GPU-kernel-friendly algorithm.
        use_lapack_panel = !(Dagger.chunktype(Ac[1,1]) <: GPUArraysCore.AbstractGPUArray)
