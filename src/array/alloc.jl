...
function stage(ctx, A::AllocateArray)
    tasks = emit_chunk_tasks!(A.domainchunks, A.procgrid, A.eltype,
        (scope, I, i) -> begin
        x = A.domainchunks[I]
        nd = ndims(A.domainchunks)
        # Default `return_type` is dense `Array`, matching the pre-sparse
        # allocator. Sparse/GPU callers that produce another tile type pass
        # `return_type` explicitly (see `spzeros` / `allocate_tiled`).
        ret_type = something(A.return_type, Array{A.eltype, nd})
        if A.want_index
