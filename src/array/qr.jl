...
function LinearAlgebra.rmul!(A::StridedVecOrMat{T}, B::AdjointQ{<:Any, <:QRCompactWYQ{T, <:DMatrix{T}, <:DMatrix{T}}}) where {T<:Union{Float32,Float64,ComplexF32,ComplexF64}}
    trans = T <: Complex ? 'C' : 'T'
    return _apply_dense_qr!('R', trans, B.Q, A)
end
function LinearAlgebra.rmul!(A::DVector{T}, B::AdjointQ{<:Any, <:QRCompactWYQ{T, <:DMatrix{T}, <:DMatrix{T}}}) where {T<:Union{}}
    trans = T <: Complex ? 'C' : 'T'
