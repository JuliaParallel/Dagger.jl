...
Base.@kwdef mutable struct ThunkSpec
    ...
    eager_ref::Union{DRef,Nothing} = nothing
...
mutable struct Thunk
    ...
    function Thunk(spec::ThunkSpec)
        return new(spec.fargs,
                   ...
                   spec.eager_ref, spec.options, spec.propagates,
                   true, true)
    end
end
function Thunk(f, xs...;
...
function args_kwargs_to_pairs(f, args, kwargs)
    ...
    for idx in 1:length(kwargs)
        kwarg = kwargs[idx]
        push!(args_kwargs, Argument(kwarg[1], kwarg[2]))
    end
    return args_kwargs
end
...
function _delayed(f, options::Options)
    (args...; kwargs...) -> Thunk(args_kwargs_to_pairs(f, args, kwargs)...; options.options...)
end
...
function spawn(f, args...; kwargs...)
    ...
    args_kwargs = args_kwargs_to_pairs(f, args, kwargs)
    ...
    if lvl > 0
        ...
        for arg in inputs(t)[2:end]
            ...
            if ispositional(arg)
                ...
                push!(t_inputs, arg_kw(arg) => input)
            end
        end
