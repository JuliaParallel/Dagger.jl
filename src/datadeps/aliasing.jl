...
function generate_slot!(state::DataDepsState, dest_space, data)
    ...
    data_chunk = if reused !== nothing
        ...
        hier_log!(LogHierSlot, 0x01, LogHierSlotId(:reused), time_ns() - t0)
...
# and the `alloc_header` (dimensions, for an array) cross the wire; `data`
# deliberately does not, which is the whole point (a `remotecall_fetch` closing
# over it would serialize the payload).
function remotecall_endpoint_alloc(accel::DistributedAcceleration, to_proc, to_space, ::Type{T}, header) where T
    wid = root_worker_id(to_proc)
    if wid == myid()
        return tochunk(alloc_uninit(to_space, T, header), to_proc, to_space)
    end
    return remotecall_fetch(wid, to_proc, to_space, T, header) do to_proc, to_space, T, header
        return tochunk(alloc_uninit(to_space, T, header), to_proc, to_space)
    end
end
    ...
    if parts === nothing
        ...
        if slot_may_be_uninit(from_space, to_space, T_dest)
            return aliased_object!(cache, data) do data
                return remotecall_endpoint_alloc(accel, to_proc, to_space, T_dest, alloc_header(data))
            end
        end
        return aliased_object!(cache, data) do data
