...
function report_hier_stats(ntasks::Int, nargs::Int, npartitions::Int)
    ...
    reused_ns = UInt64(0); reused_count = 0
    ...
    stats = (;ntasks, nargs, npartitions,
             ...
             reused_ns, reused_count, ainfo_ns, ainfo_count, samples=slot_samples)
...
function _print_hier_stats(stats)
    ...
    datadeps plan: $(stats.ntasks) tasks, $(stats.nargs) args, $(stats.npartitions) partitions, $(ms(total)) ms total
      ...
      phase 4 schedule  $(ms(stats.schedule_ns)) ms  ($(stats.slot_count) slots, $(ms(stats.slot_ns)) ms, of which $(stats.moved_count) moved data in $(ms(stats.moved_ns)) ms, $(stats.samespace_count) of those within one space in $(ms(stats.samespace_ns)) ms, and $(stats.reused_count) reused from the slot cache in $(ms(stats.reused_ns)) ms)
