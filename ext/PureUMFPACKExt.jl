...
function Dagger.BlockUMFPACKPreconditioner(A::DMatrix; kwargs...)
    ...
    return Dagger._build_block_preconditioner(Dagger.BlockUMFPACKPreconditioner, A, build)
end

# `PureLU` provides `\` but no `ldiv!`, and it does not subtype `Factorization`,
# so the `\` path has to be selected explicitly.
Dagger._apply_inverse!(y, F::PureUMFPACK.PureLU, x) = copyto!(y, F \ x)
