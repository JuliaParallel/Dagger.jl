...
GPU_SELECTION
  ...
  [0]           → only device 0 (0-based CUDA device ID)
  [0, 1]        → devices 0 and 1 individually + combined multi-GPU scope
...
for dep in ["CUDA", "Dagger", "Statistics", "Printf", "Dates"]
    haskey(Pkg.project().dependencies, dep) || Pkg.add(dep)
end

using CUDA, Dagger, Statistics, Printf, Dates

# ═══════════════════════════════════════════════════════════════════════════════
#  PARAMETERS  ←  edit this block to configure everything
# ═══════════════════════════════════════════════════════════════════════════════

# Which GPU device IDs (0-based) to benchmark.
# :all  → auto-detect every GPU and also run a combined multi-GPU scope
# [0]   → only GPU 0
# [0,1] → GPU 0 and GPU 1 individually, plus a joint multi-GPU scope
const GPU_SELECTION = [0]
...
# ═══════════════════════════════════════════════════════════════════════════════
#  GPU discovery
#  Uses Dagger.gpu_processor(Val(:CUDA)) so we never import CUDAExt directly.
# ═══════════════════════════════════════════════════════════════════════════════

function discover_gpu_procs()
    GpuProc = Dagger.gpu_processor(Val(:CUDA))   # returns CuArrayDeviceProc type
    procs_set = filter(p -> p isa GpuProc, Dagger.get_processors(Dagger.OSProc()))
    
    # Convert the Set to a Vector using collect() and sort it
    sorted_procs = sort(collect(procs_set); by = p -> p.device)
...
# ═══════════════════════════════════════════════════════════════════════════════
#  Scope helpers
#  CUDAExt uses 1-based device indices for Dagger scopes; proc.device is 0-based.
# ═══════════════════════════════════════════════════════════════════════════════

single_gpu_scope(device_id::Int)          = Dagger.scope(; cuda_gpu  = device_id + 1)
multi_gpu_scope(device_ids::Vector{Int})  = Dagger.scope(; cuda_gpus = device_ids .+ 1)
...
function bench_function(f::Function, procs::Vector;
    ...
    # Sync every named stream on every participating GPU
    gpu_sync!() = (foreach(Dagger.gpu_synchronize, procs); CUDA.synchronize())

    # Two-pass GC ensures DRef finalizers fire before pool reclaim
    reclaim!() = (GC.gc(false); GC.gc(true); CUDA.reclaim())
    ...
    device_ids   = [p.device for p in procs]
...
struct BenchResult
    ...
    util_peak   ::Float64   # peak GPU compute utilization % (NaN if NVML unavailable)
...
function enough_vram(procs::Vector, min_gib = MIN_FREE_VRAM_GIB)
    for p in procs
        gpu_vram_info(p.device).free_gib < min_gib && return false
    end
    return true
end

function force_reclaim!(procs::Vector)
    prev_free = CUDA.available_memory()
    while true
        GC.gc(false)
        GC.gc(true)
        foreach(Dagger.gpu_synchronize, procs)   # sync every GPU in the scope
        CUDA.reclaim()
        curr_free = CUDA.available_memory()
...
function print_header()
    println(banner('═'))
    println("  DAGGER.jl MULTI-GPU BENCHMARK  ·  $(Dates.format(now(), "yyyy-mm-dd HH:MM:SS"))")
    println("  Julia $(VERSION)  ·  CUDA.jl $(pkgversion(CUDA))  ·  Dagger.jl $(pkgversion(Dagger))")
    println(banner('═'))
end
...
function print_gpu_table(all_procs)
    ...
    println(banner('─'))
    for p in all_procs
        info = gpu_vram_info(p.device)
        @printf("  [%d]  %-34s  %5.1f GiB total  %5.1f GiB free\n",
            p.device, CUDA.name(CUDA.CuDevice(p.device)),
...
function print_footer(results::Vector{BenchResult}, total_s::Float64 = 0.0, n_skipped::Int = 0)
    ...
    valid_util = filter(!isnan, [r.util_peak for r in results])
    if !isempty(valid_util)
        r_util = results[argmax([isnan(r.util_peak) ? -Inf : r.util_peak for r in results])]
        @printf("  🔥 Peak GPU util:   %s  (%s  %s  blk=%s)\n",
            util_str(r_util.util_peak), r_util.op, r_util.scope_label,
            "$(r_util.blk_sz[1])×$(r_util.blk_sz[2])")
        overall_util = mean(filter(!isnan, [r.util_mean for r in results]))
        @printf("  📈 Avg GPU util:    %s  across all configs\n", util_str(overall_util))
    end
...
struct ScopeEntry
    ...
    procs ::Vector           # CuArrayDeviceProcs used — needed for gpu_synchronize
end

function build_scope_entries(all_procs, selection)
    device_ids = selection === :all ? [p.device for p in all_procs] : selection

    available = Set(p.device for p in all_procs)
    for id in device_ids
    ...
    if length(device_ids) == 1
        ...
        id   = only(device_ids)
        proc = only(filter(p -> p.device == id, all_procs))
    ...
    elseif selection === :all
        ...
        multi_procs = filter(p -> p.device in device_ids, all_procs)
...
function main()
    CUDA.functional() || error("CUDA is not functional. Check your driver and CUDA installation.")

    all_procs = discover_gpu_procs()
    isempty(all_procs) &&
        error("No CUDA GPU processors found in Dagger. Is the CUDAExt extension loaded?")

    cudaext = Base.get_extension(Dagger, :CUDAExt)
    cudaext.stream_strategy!(STREAM_STRATEGY)
