...
@testset "CUDA" begin
    if !Dagger.gpu_can_compute(:CUDA)
        ...
        cuproc = if isdefined(Base, :get_extension)
            Base.get_extension(Dagger, :CUDAExt).CuArrayDeviceProc
        else
            CuArrayDeviceProc
        end
...
@testset "ROCm" begin
    if !Dagger.gpu_can_compute(:ROC)
        ...
        rocproc = if isdefined(Base, :get_extension)
            Base.get_extension(Dagger, :ROCExt).ROCArrayDeviceProc
        else
            ROCArrayDeviceProc
        end
...
@testset "oneAPI" begin
    if !Dagger.gpu_can_compute(:oneAPI)
        ...
        oneproc = if isdefined(Base, :get_extension)
            Base.get_extension(Dagger, :IntelExt).oneArrayDeviceProc
        else
            oneArrayDeviceProc
        end
...
@testset "Metal" begin
    if !Dagger.gpu_can_compute(:Metal)
        ...
        mtlproc = if isdefined(Base, :get_extension)
            Base.get_extension(Dagger, :MetalExt).MtlArrayDeviceProc
        else
            MtlArrayDeviceProc
        end
