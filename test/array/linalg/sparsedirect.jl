...
@testset "Sparse direct solvers" begin
    ...
    @testset "block direct preconditioner ($(name))" for (name, build) in (
        ...
        # A non-square block grid is re-tiled to the finer square blocks rather
        # than rejected, so the exact block-diagonal inverse it applies is the
        # one over `Blocks(k÷2, k÷2)`.
        kf = k ÷ 2
        yref_fine = similar(b)
        for s in 1:kf:n
            r = s:min(s + kf - 1, n)
        ...
        P_ragged = build(distribute(Asp, Blocks(k, kf)))
        Db_ragged = distribute(b, Blocks(kf))
