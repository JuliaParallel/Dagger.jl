...
function dynamic_add_thunk(x)
    h = sch_handle()
    t = Dagger.Sch.add_thunk!(h, nothing=>x) do y
        y+1
    end
    wait(t)
    return fetch(t)
...
function dynamic_add_thunk_self_dominated(x)
    h = sch_handle()
    t = Dagger.Sch.add_thunk!(h, nothing=>h.thunk_id, nothing=>x) do y
        y+1
    end
    return fetch(t)
end
...
@testset "Scheduler" begin
    ...
    @testset "Thunk options" begin
        ...
        @testset "allow errors" begin
            ...
            @test_throws_unwrap (Dagger.ThunkFailedException, ErrorException) collect(a)
        end
    end
