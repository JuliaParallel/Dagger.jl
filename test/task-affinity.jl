@testset "Task affinity" begin
    ...
    function get_execution_scope(x::DTask)
        ...
        return intersect_scopes(compute_scope, result_scope, f_scope, inputs_scopes...)
    end
    ...
    function is_equal_scope(scope1::Dagger.AbstractScope, scope2::Dagger.AbstractScope)
        scope1_procs = Dagger.compatible_processors(scope1)
        scope2_procs = Dagger.compatible_processors(scope2)
        return issetequal(scope1_procs, scope2_procs)
    end
    function is_equal_scope(scopes::Dagger.AbstractScope...)
        scope1 = scopes[1]
        scope1_procs = Dagger.compatible_processors(scope1)
    ...
    function is_subset_scope(scope1::Dagger.AbstractScope, scope2::Dagger.AbstractScope)
        scope1_procs = Dagger.compatible_processors(scope1)
        scope2_procs = Dagger.compatible_processors(scope2)
    ...
    @testset "scope, compute_scope and result_scope" begin
        ...
        @testset "scope" begin
            ...
            @test is_subset_scope(get_execution_scope(task1), scope_only)
        end
        ...
        @testset "compute_scope" begin
            ...
            @test is_subset_scope(get_execution_scope(task1), compute_scope_only) &&
                  is_subset_scope(get_execution_scope(task2), compute_scope_only)
        end
        ...
        @testset "result_scope" begin
            ...
            @test is_subset_scope(get_execution_scope(task1), result_scope_only)
        end
        ...
        @testset "compute_scope and result_scope with intersection" begin
            if numscopes >= 3
                ...
                all_scope_intersect      = intersect_scopes(compute_scope_intersect, result_scope_intersect)
                ...
                @test is_subset_scope(get_execution_scope(task1), all_scope_intersect) &&
                      is_subset_scope(get_execution_scope(task2), all_scope_intersect) &&
                      is_subset_scope(get_execution_scope(task3), all_scope_intersect)
            end
        end
    ...
    @testset "Chunk function, scope, compute_scope and result_scope" begin 
        ...
        @testset "scope" begin
            ...
            all_scope = intersect_scopes(scope_only, chunk_scope)
            ...
            @test get_result_scope(task1)  == Dagger.AnyScope()
            @test get_final_result_scope(task1) == Dagger.AnyScope()
            @test is_equal_scope(get_execution_scope(task1), all_scope)
        end
        ...
        @testset "compute_scope" begin
            ...
            chunk_proc         = rand(availprocs)
            chunk_scope        = Dagger.UnionScope(scope_b..., scope_c...)
            all_scope          = intersect_scopes(compute_scope_only, chunk_scope)
            ...
            @test get_result_scope(task1)  == get_result_scope(task2)  == Dagger.AnyScope()
            @test get_final_result_scope(task1) == get_final_result_scope(task2) == Dagger.AnyScope()
            @test is_equal_scope(get_execution_scope(task1),
        ...
        @testset "result_scope" begin
            ...
            chunk_proc         = rand(availprocs)
            chunk_scope        = Dagger.UnionScope(scope_b..., scope_c...)
            all_scope = intersect_scopes(result_scope_only, chunk_scope)
        ...
        @testset "compute_scope and result_scope with intersection" begin
            if length(availscopes) >= 3
                ...
                chunk_proc               = rand(availprocs)
                chunk_scope              = Dagger.UnionScope(scope_b..., scope_c...)
                all_scope = intersect_scopes(compute_scope_intersect, result_scope_intersect, chunk_scope)
    ...
    @testset "Chunk arguments, scope, compute_scope and result_scope with intersection of chunk arg and scope" begin 
        ...
        @testset "scope" begin
            ...
            all_scope   = intersect_scopes(scope_only, arg_scope)
            ...
            @test is_equal_scope(get_execution_scope(task11), all_scope)
        end

        @testset "compute_scope" begin
            compute_scope_only = Dagger.UnionScope(scope_b..., scope_c...)
            scope              = Dagger.UnionScope(scope_b..., scope_c...)
            all_scope          = intersect_scopes(compute_scope_only, arg_scope)
            ...
            @test is_equal_scope(get_execution_scope(task11),
        ...
        @testset "result_scope" begin
            ...
            all_scope          = intersect_scopes(result_scope_only, arg_scope)
        ...
        @testset "compute_scope and result_scope with intersection" begin
            ...
            if length(scope_bc) >= 3
                ...
                all_scope                = intersect_scopes(compute_scope_intersect, result_scope_intersect, arg_scope)
